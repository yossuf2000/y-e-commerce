...
class YECommerceSupabaseUser extends BaseAuthUser {
  YECommerceSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> yECommerceSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = YECommerceSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
